<template>
    <div class="card mt-5">
        <DataTable :value="$store.state.testItems" :paginator="true" class="" :rows="5"
            dataKey="id" :rowHover="true" v-model:selection="selectedItem"  :loading="loading"
        ...
        <div class="flex justify-content-end">
            <Button label="Next" class="m-3" icon="pi pi-chevron-right" iconPos="right" @click="handleNextBtn" :disabled="!selectedItem" />
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        handleNextBtn() {
            ...
            console.log('test set', this.$store.state.currentTestItem)
            this.$router.push({name: 'edit-sentence', params: { setid: this.$store.state.currentTestSet.id}})
        }
    }
}
</script>